...
```bash
ls -la hwp-platform/docs/clients/{slug}/data-inventory.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
Expected: `EXISTS`.
...
```bash
grep -rn "politique-de-confidentialite\|politica-de-privacidad\|privacy" hwp-platform/apps/{slug}/src --include="*.tsx" | grep -i "form\|input\|booking\|contact"
```
...
```bash
grep -rni "deletion\|erasure\|art.17\|right to" hwp-platform/docs/clients/{slug}/ 2>/dev/null
```
...
```bash
grep -rn "input\|field\|FormData\|z\.string\|z\.email\|z\.number" hwp-platform/apps/{slug}/src --include="*.tsx" --include="*.ts" | grep -v "node_modules"
```
...
```bash
grep -rn "newsletter\|marketing\|promotional" hwp-platform/apps/{slug}/src --include="*.tsx" --include="*.ts" -i
```
...
**WordPress equivalent:** el plugin de GDPR (CookieYes, GDPR Cookie Consent) más tener la página de "Política de privacidad" creada y linkeada desde el footer. En HWP lo mismo — pero sin plugin que lo haga automático.

**Day-to-day impact:** antes de que cualquier site HWP vea tráfico real, estas casillas tienen que estar marcadas. El `security-specialist` las audita en el pre-deploy check. Si alguna está en rojo, el site no se despliega — punto.
