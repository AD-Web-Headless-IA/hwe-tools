...
| generate-block-story | alpha | 0.1.0 | `.claude/skills/generate-block-story/` | Generate a complete, developer-ready user story for a NEW base block in `@hwe/core-ui`, aligned with real repo patterns, SPECBOOT and TDD. Reads architecture/contracts/specs + closest existing blocks, then writes a self-contained spec to `docs/plans/base-blocks/stories/US-{NNN}-{slug}.md` (canonical stories home) for team review. Block-specific counterpart of `/enrich-us`; feeds `/scaffold-block --target base` and `/propose`. DEC-008 + DEC-025. |
...
| setup-booking | alpha | 0.2.0 | `.claude/skills/setup-booking/` | Onboard a client site for a booking engine: writes `booking` into `client.config.ts`, adds the engine's CSP domains to `next.config.mjs`, scaffolds the `[data-engine]` CSS override section, ensures `TenantProvider` wraps the app, and optionally adds `BookingSearchBlock` (`--with-block`) / `BookingFavoritesBlock` (`--with-favorites`) / `BookingSimpleBlock` (`--with-simpleblock --categories <ids>`) instances, toggling the matching `booking.features`. Params discriminated by engine. DEC-025 + DEC-027. |
