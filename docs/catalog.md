...
| setup-booking | alpha | 0.2.0 | `.claude/skills/setup-booking/` | Onboard a client site for a booking engine: writes `booking` into `client.config.ts`, adds the engine's CSP domains to `next.config.mjs`, scaffolds the `[data-engine]` CSS override section, ensures `TenantProvider` wraps the app, and optionally adds `BookingSearchBlock` (`--with-block`) / `BookingFavoritesBlock` (`--with-favorites`, toggles `booking.features.favorites`) instances. Params discriminated by engine. DEC-025 + DEC-027. |
