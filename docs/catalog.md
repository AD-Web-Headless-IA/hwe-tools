...
| setup-booking | alpha | 0.1.0 | `.claude/skills/setup-booking/` | Onboard a client site for a booking engine: writes `booking` into `client.config.ts`, adds the engine's CSP domains to `next.config.mjs`, scaffolds the `[data-engine]` CSS override section, ensures `TenantProvider` wraps the app, and optionally adds a `BookingSearchBlock` instance. Params discriminated by engine. DEC-025. |
