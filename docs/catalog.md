...
| scaffold-block | alpha | 0.2.0 | `.claude/skills/scaffold-block/` | Scaffold a new block: use `--target base` (default) for `packages/core-ui/src/base-blocks/` or `--target client` for `apps/{slug}/src/blocks/`. Creates the folder + 5 mandatory files from templates per `docs/contracts/frontend/block-contract.md`. DEC-015 pre-flight check on Step 2. |
| scaffold-site | alpha | 0.2.0 | `.claude/skills/scaffold-site/` | Bootstrap a new client site repository from the standard template: creates `src/blocks/registry.ts`, `src/app/globals.css`, `src/theme/tokens.json`, `tailwind.config.ts`, `client.config.ts`, and `next.config.mjs` following DEC-011 + DEC-015 structure. Reads `import-figma` outputs (tokens.json, language). |
...
| senior-developer | alpha | 0.1.0 | `.claude/agents/senior-developer.md` | Sonnet. Core package expert — reference for existing patterns in `packages/*/src/`. Guides implementer on idioms and conventions. |
...
| composition-rules | alpha | 0.1.0 | `packages/core-ui/src/composition-rules/` | Runtime + build-time module that defines block ordering constraints, co-occurrence rules, and slot compatibility for page compositions. Consumed by Payload CMS editor and the BlockRenderer. |
| coding-standards | alpha | 0.1.0 | `docs/specs/frontend/coding-standards.md` | HWP frontend coding rules: component structure, import order, no-enum, React patterns, anti-patterns table. Extends base-standards + frontend-standards. |
