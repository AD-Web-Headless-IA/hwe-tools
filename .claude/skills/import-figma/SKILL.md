...
- One git repo per client at `figma-makes/{slug}/` — each is the original `git clone`, with its own `.git/`, independent from the `hwp-platform/` repo. Never put `figma-makes/` inside `hwp-platform/`.
...
- All generated artifacts (analysis, notes) live OUTSIDE the cloned repo — under `hwp-platform/docs/clients/{slug}/` — so the clone stays pristine and `git pull` never has conflicts with our untracked files.
...
- Workspace root: `C:\laragon\www\Hospitality Web Platform\`. The `figma-makes/` container lives there, as a sibling of `hwp-platform/`.
...
### Step 5 — Write `hwp-platform/docs/clients/{slug}/figma-analysis.md`

Create the directory `hwp-platform/docs/clients/{slug}/` if it doesn't exist.
...
### Step 6 — Write `hwp-platform/docs/clients/{slug}/figma-notes.md`
...
> **Phase 1 note:** These files live provisionally in `hwp-platform/docs/clients/{slug}/`. When the `site-{slug}/` repo is created, move them to `site-{slug}/docs/architecture/`. The `figma-makes/{slug}/` repo itself stays where it is — it's the canonical visual reference history.
...
3. Copy the template from `hwp-platform/docs/clients/_template/design-language.md`.
4. Fill in every pattern field with the observed values. Mark uncertain items with `(?)`.
5. Write the filled template to `hwp-platform/docs/clients/{slug}/design-language.md`.
...
3. Write to `hwp-platform/docs/clients/{slug}/tokens.json`.
...
Files generated:
  - hwp-platform/docs/clients/{slug}/figma-analysis.md   (includes ## Language section)
  - hwp-platform/docs/clients/{slug}/figma-notes.md
  - hwp-platform/docs/clients/{slug}/design-language.md  ← DRAFT, review before use
  - hwp-platform/docs/clients/{slug}/tokens.json         ← DRAFT, validate before use
...
Files generated:
  - hwp-platform/docs/clients/hotel-balneario-fuente-de-cabriel/figma-analysis.md
  - hwp-platform/docs/clients/hotel-balneario-fuente-de-cabriel/figma-notes.md
```
