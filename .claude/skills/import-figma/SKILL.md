...
You are a design system analyst. Your job is to import (or re-import) a Figma Make repository as a visual reference, keep one isolated git repo per client under `figma-makes/{slug}/`, tag every import so we never lose history, extract its design tokens and block catalog, and write provisional client context files that Claude Code can read when building the equivalent site with `@hwe/core-ui`.

Success means: a developer can build the site using `@hwe/core-ui` blocks by reading only the generated `figma-notes.md`, without ever opening the Figma Make repo manually — and a year from now we can `git checkout import-YYYY-MM-DD` inside any client's repo to see exactly what the designer shipped that day.
...
- All generated artifacts (analysis, notes) live OUTSIDE the cloned repo. Target: `site-{slug}/docs/` if the client repo exists, otherwise `hwe-tools/docs/clients/{slug}/` as a provisional location until the client repo is created.
...
- Workspace root: the directory the developer has open — its name varies per machine. See `docs/contracts/general/workspace-structure.md`. The `figma-makes/` container lives at the workspace root as a plain folder (not a git repo).
...
Ensure the container exists: `mkdir -p "figma-makes"` (relative to workspace root).

### Step 2 — Clone or re-import

Let `targetDir = figma-makes/{slug}/` (relative to workspace root).
...
> You are a static analyzer for a React/Figma-Make repository at `figma-makes/{slug}/` (relative to the workspace root). Read its files and produce a structured analysis.
>
> ## What to extract
>
...
- Otherwise: write to `hwe-tools/docs/clients/{slug}/figma-analysis.md` (provisional).

Create the directory if it doesn't exist.

From the sub-agent's JSON, generate the analysis file in the target output directory determined in Step 5:
...
## Notes for @hwe/core-ui
{Observations about conventions, design decisions, or patterns relevant when building the equivalent blocks in @hwe/core-ui. Mention any unusual choices that should be preserved or any anti-patterns to avoid.}
```
...
> Visual context for Claude Code when building with @hwe/core-ui.
> Auto-generated by /import-figma — do not edit manually.
...
> **DEC-017 note:** When the `site-{slug}/` repo exists, files live in `site-{slug}/docs/`. Provisional location in `hwe-tools/docs/clients/{slug}/` is only used before the client repo is created. The `figma-makes/{slug}/` repo stays where it is — it's the canonical visual reference history.
...
3. Copy the template from `hwe-tools/.claude/templates/design-language.md` (relative to the client repo root, where `hwe-tools/` is the submodule).
...
Files generated (in site-{slug}/docs/ if repo exists, else hwe-tools/docs/clients/{slug}/):
...
```
Repo: figma-makes/base-template/   (first import)
Tag created: import-2026-06-04
Client detected: Base Template (base-template)
Tokens extracted: 6 colors, 2 font families
Catalog: 11 blocks, 2 pages, 48 UI primitives
Files generated (in site-demo/docs/ if repo exists, else hwe-tools/docs/clients/base-template/):
