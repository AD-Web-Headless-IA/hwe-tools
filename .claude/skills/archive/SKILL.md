...
You are **read-only on source code** (`packages/`, `apps/`). You write only to `docs/` and the story file itself.
...
- **Never touch:** `docs/architecture/decisions.md` (Architect only), `docs/contracts/` (Architect approval required), any `.claude/agents/` or `.claude/skills/` file, any file in `packages/` or `apps/`.
...
- `hwp-platform/docs/specs/ai/specboot-flow.md` — the full pipeline context
- The story file (`$0`) verbatim — to understand what was built and what changed
- The proposal artifact at `hwp-platform/docs/plans/{epic}/proposals/US-{NNN}-proposal.md` — to detect divergence from implementation
...
- File exists at `hwp-platform/{STORY_PATH}`.
...
- `PROPOSAL_PATH` = `hwp-platform/docs/plans/{EPIC}/proposals/{US_ID}-proposal.md`.
...
Spec files are under `hwp-platform/docs/specs/`. Only edit files whose domain was touched by this story.

### Step 3 — Update catalog.md

Open `hwp-platform/docs/catalog.md`. For each item created, bumped, or deprecated by this story:
...
Open `hwp-platform/docs/guides/project-map.md` (if it exists; skip if absent).
...
Edit the story file (`hwp-platform/{STORY_PATH}`):
...
```bash
git add -- hwp-platform/docs/ hwp-platform/{STORY_PATH}
```
