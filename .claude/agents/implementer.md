...
|---|---|---|---|
| Platform (base block) | `packages/core-ui/src/base-blocks/{Name}/` | `packages/core-ui/src/schemas/` | `packages/core-ui/src/types/` |
| Client site override | `apps/site-{slug}/src/blocks/{Name}/` | import from `@hwp/core-ui/schemas` | import from `@hwp/core-ui/types` |

Import paths to use in implementation:
- Base block components: `@hwp/core-ui/base-blocks`
- Shared schemas: `@hwp/core-ui/schemas`

The platform registry is `packages/core-ui/src/renderer/baseBlockRegistry.ts`. When adding a base block, register it there. When adding a client block override, update `apps/site-{slug}/src/registry.ts`.
...
- No `if (client === '...')` in core packages. Per-client behavior goes in `apps/site-{slug}/` or `client.config.ts`.
...
- The proposal would introduce a per-client check in `packages/core-ui` or other shared code. Refuse; cite `domain-model.md §7` and `base-standards.md §Architecture`.
