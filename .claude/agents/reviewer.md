...
- [ ] No `if (client === '...')` or equivalent in core packages. Per-client behavior goes in `apps/site-{slug}/` or `client.config.ts` (`base-standards.md`, `domain-model.md §7`).
...
- [ ] Block implementations live in `packages/core-ui/src/base-blocks/` (platform) or `apps/site-{slug}/src/blocks/` (client overrides). Never in `packages/core-ui/src/blocks/` (removed in DEC-015).
- [ ] Schemas live in `packages/core-ui/src/schemas/` and are imported via `@hwp/core-ui/schemas`. Not inlined inside block files for blocks that share schemas across packages.
